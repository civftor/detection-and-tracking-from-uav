...
/* ******************************************* */
            FILE *lfp = fopen("yolov3_weights_saves_log.txt", "a");
            fprintf(lfp, "%ld: %f, %f avg, %f rate, %lf seconds, %d images\n", get_current_batch(net), loss, avg_loss, get_current_rate(net), what_time_is_it_now()-time, i*imgs);
            fclose(lfp);
/* ******************************************* */
...
void validate_detector_recall(char *cfgfile, char *weightfile)
{
    ...
    list *plist = get_paths("cfg/aerial-valid.txt");
    char **paths = (char **)list_to_array(plist);
...
void calc_anchors(char *datacfg, int num_of_clusters, int width, int height)
{
    ...
    if (width < 0 || height < 0) {
        printf("Usage: darknet detector calc_anchors data/voc.data -width 416 -height 416 \n");
    ...
    char buff[1024];
    FILE* fw = fopen("cfg/anchors.txt", "wb");
    if (fw) {
        printf("\nSaving anchors to the file: cfg/anchors.txt \n");
        printf("anchors = ");
        ...
        printf(" Error: file cfg/anchors.txt can't be open \n");
    }
...
void run_detector(int argc, char **argv)
{
    ...
    //int num_of_clusters = find_int_arg(argc, argv, "-num_of_clusters", 9);
    if(argc < 4){
    ...
    //else if(0==strcmp(argv[2], "anchors")) calc_anchors(datacfg, num_of_clusters, width, height);
