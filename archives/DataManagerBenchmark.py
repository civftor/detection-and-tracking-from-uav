...
class DataManager:
    ...
    def __scan_paths( self, videos_path, labels_path, subset ):
        assert os.path.exists( videos_path ), "Data path does not exists: " + videos_path
        assert os.path.exists( labels_path ), "Labels path does not exists: " + labels_path
        ...
        for i, d in enumerate( dataset['videos'] ):            
            ...
            dataset['labels'].append( os.path.join( labels_path, d + "_gt_whole.txt" ) )
    ...
    def __get_label_at(self, i, j):
        ...
        index = int( self._dataset['frames'][i][j][-10:-4] )
